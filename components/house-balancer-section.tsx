...
const CONSOLIDATED_GROUPS = {
  KINGS: {
    label: "KING",
    codes: ["RM1K0006", "RM1KA0008", "RM1KA0009"],
    upgradeTarget: null,
    isUpgrade: false,
  },
  QUEENS: {
  ...
  VIQN: {
    label: "View Queens",
    codes: ["RM2Q0002", "RM2QA003", "RM2QA004", "RM2QA005"],
    upgradeTarget: "QUEENS",
    isUpgrade: true,
  },
  SUITES: {
    label: "SUIT",
    codes: ["SU1B0010", "SU1BA0011"],
    upgradeTarget: "KINGS",
    isUpgrade: true,
  },
} as const
...
const ROOM_CODE_TO_GROUP = new Map<string, ConsolidatedCategory>(
  (Object.entries(CONSOLIDATED_GROUPS) as [ConsolidatedCategory, typeof CONSOLIDATED_GROUPS[ConsolidatedCategory]][])
    .flatMap(([group, config]) => config.codes.map((code) => [code, group] as const))
)
...
export function HouseBalancerSection({ onBalanceComplete }: HouseBalancerSectionProps) {
  ...
  const normalizeRoomCode = (value: string) => {
    const firstToken = value.replace(/"/g, "").trim().split(/\s+-\s+/)[0]?.trim().toUpperCase() || ""
    const match = firstToken.match(/^(RM[A-Z0-9]+|SU[A-Z0-9]+|ROH)\b/)
    return match?.[1] || ""
  }

  const parseAvailabilityValue = (value: string | undefined) => {
    const normalized = String(value ?? "").replace(/[",%\s]/g, "")
    const parsed = Number(normalized)
    return Number.isFinite(parsed) ? parsed : 0
  }

  const findAvailabilityColumn = (headers: string[]) => {
    ...
    const dateIndex = headers.findIndex((header, index) => index > 0 && datePatterns.some((pattern) => header.includes(pattern)))
    if (dateIndex >= 0) return dateIndex

    const firstDateColumn = headers.findIndex((header, index) => index > 0 && /\b\d{1,2}\/\d{1,2}\b/.test(header))
    return firstDateColumn >= 0 ? firstDateColumn : 2
  }

  // Parse Room Availability CSV. This is the only source for bucket availability.
  const parseRoomAvailabilityCSV = useCallback((content: string) => {
    const lines = content.trim().split(/\r?\n/).filter(Boolean)
    const headers = splitCsvLine(lines[0] || "").map(h => h.replace(/"/g, '').trim())
    
    const todayColIndex = findAvailabilityColumn(headers)
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const todayValue = parseAvailabilityValue(values[todayColIndex])
      const roomCode = normalizeRoomCode(roomCodeLine)

      if (!roomCode || roomCode === "ROH" || roomCodeLine.toLowerCase().includes("run of house") || roomCodeLine.toLowerCase().includes("total")) {
        continue
      }

      const group = ROOM_CODE_TO_GROUP.get(roomCode)
      if (group) {
        roomData.push({
          roomCode,
        ...
        categoryTotals[group] += todayValue
      }
    }
  ...
  const parseGASummaryCSV = useCallback((content: string) => {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      } else if (metric === 'stayovers' || metric === 'stayover') {
        gaData.stayovers = parseInt(todayValue, 10) || 0
  ...
  const processRoomAvailabilityFile = async () => {
    ...
    try {
      ...
      const roomCodeValues = Object.fromEntries(roomData.map((room) => [room.roomCode, room.todayAvailable]))
      
      setParsedRoomData(roomData)
      setInventory(categoryTotals)
      setExtractedRoomCodeValues(roomCodeValues)
  ...
  const pendingRecommendations = recommendations.filter(r => !r.completed); useEffect(() => { if (typeof window !== "undefined") { localStorage.setItem("shuffle-recommendations", JSON.stringify(pendingRecommendations)) } }, [pendingRecommendations]); useEffect(() => { if (typeof window !== "undefined") { localStorage.setItem("shuffle-recommendations", JSON.stringify(pendingRecommendations)) } }, [pendingRecommendations])
