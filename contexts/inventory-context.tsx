...
interface InventoryContextType extends InventoryState {
  ...
  setExtractedRoomTotals: (totals: { KING: number; QUEENS: number; VIKG: number; VIQN: number; SUITES: number }, isForTomorrow?: boolean) => void
  setExtractedRoomCodeValues: (values: RoomInventory, isForTomorrow?: boolean) => void
}
...
export function InventoryProvider({ children }: { children: ReactNode }) {
  ...
  const setExtractedRoomTotals = useCallback((totals: { KING: number; QUEENS: number; VIKG: number; VIQN: number; SUITES: number }, isForTomorrow = false) => {
    setState(prev => {
      ...
      if (ROOM_CODES.VIQN[0]) targetInventory[ROOM_CODES.VIQN[0]] = totals.VIQN
  ...
  const setExtractedRoomCodeValues = useCallback((values: RoomInventory, isForTomorrow = false) => {
    setState(prev => {
      const targetInventory = isForTomorrow ? { ...prev.tomorrowInventory } : { ...prev.todayInventory }

      Object.values(ROOM_CODES).flat().forEach(code => {
        targetInventory[code] = values[code] ?? 0
      })
  ...
  const viqnTotal = calculateTotal(state.todayInventory, ROOM_CODES.VIQN)
  ...
  const tomorrowViqnTotal = calculateTotal(state.tomorrowInventory, ROOM_CODES.VIQN)
